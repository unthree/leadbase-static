...
export async function chat({ system, user, agent = false }) {
  ...
  try {
    ...
    // Network failure or a non-JSON 200 from a proxy — return the same
    // {ok:false} shape the UI expects instead of crashing the route.
    return { ok: false, model, text: `AI request failed: ${e?.message || "network error"}` };
  }
}
